...
int arraylist_new_with_capacity(arraylist** l, long capacity, arraylist_free_function* free_fn) {
	...
	if (capacity >= 0 && capacity <= ULONG_MAX) {
		if (capacity == 0) {
...
int arraylist_resize(arraylist* l, long max) {
	int new_capacity;
	...
	// if current capacity is less than half of ULONG_MAX
	// try to double the capacity, so that we have to do
	// fewer reallocations.
	// TODO: However this also means that reallocations will proceed
	// with size of 1 after array size has reached ULONG_MAX/2

	if (l->capacity < ULONG_MAX / 2) {
		//double capacity
